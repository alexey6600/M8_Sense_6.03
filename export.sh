...
if [ "$1" == "local" ]; then
	echo "Local Build"
	build=/home/alex/Android-m8/out
	export PATH=/home/alex/Android-m8/linaro-4.9/bin:$PATH
else
	echo "Remote Build"
	build=/home/savoca/downloads/furnace/m8
...
cmdline="console=ttyHSL0,115200,n8 androidboot.hardware=qcom user_debug=31 ehci-hcd.park=3 zcache"
...
if [ -f out/"$kerneltype" ]; then
	mkbootimg --kernel out/"$kerneltype" --ramdisk out/initramfs.img --cmdline "$cmdline" --pagesize $ps --base $base --kernel_offset $kernel_offset --ramdisk_offset $ramdisk_offset --tags_offset $tags_offset --dt out/dt.img --output ozip/boot.img
else
